	 ...
	 if( srv_connchk() == "C" ) {
	   if( $G_user_type == "ROOT" ) {
			...
			isset($_REQUEST["add_months"]) )
		{
			...
			if($_REQUEST["add_months"] == 240)
			{
				$coverage_end = mktime(0, 0, 0,substr($current_date, 4, 2),substr($current_date, 6, 2)+1,substr($current_date, 0, 4));
	  ...
	  // comiensa el fslb insertar
			...
			isset($_REQUEST["add_months"]) )
		{
			...
			// IT comes to this line			echo"HIT2HITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHIT";
            //EVEN THOUGH ADD_MONTHS IS NOT 240 IT GOES INTO DEMO
			//echo "Valor de add_months: " . $_REQUEST["add_months"];
			
			if($_REQUEST["add_months"] != 240) // Si no es demo entonces se le suman los meses señleccionados
			{
				$coverage_end = $coverage_start + ((3600 * 24 * $G_days_per_month) * (int)$_REQUEST["add_months"]);
				$mesesdbit = $_REQUEST["add_months"];
				//echo "ESTO NO ES DEMO!!!";
			}
			
			if($_REQUEST["add_months"] == 240) // Si la cuenta es de tipo demo sumar un dia a la fecha actual
			{
				$coverage_end = mktime(0, 0, 0,substr($current_date, 4, 2),substr($current_date, 6, 2)+1,substr($current_date, 0, 4));
				$mesesdbit = 0;
				//echo "ESTO  ES DEMO!!!";
		...
		//echo "Valor de add_months: " . $_REQUEST["add_months"];
		//echo " Balance de user: " . get_credit_balance( $G_username );
		//echo " INT meses a restar: " . (int)$_REQUEST["add_months"];
		//$saldo = get_credit_balance( $G_username ) - (int)$_REQUEST["add_months"];
		//echo " Saldo final de la operacion: " . $saldo; // saldo es -1 entonces por q esta insertando ? 
		
		
		
			// Insertar el usuario si es que estamos hablando de DEMO o si la resta del credito menos los meses a dar deja el saldo en 0 o arriba
			if( $_REQUEST["add_months"] == 240  || (get_credit_balance( $G_username ) - (int)$_REQUEST["add_months"]) >= 0)
			{
				// -- check if account already exists
				if( (int)query_scalar("select count(*) from accounts where upper(account) = '".strtoupper($_REQUEST["add_username"])."'") == 0 )
				{
							...
							"expires" => date("Y-m-d", $coverage_end)) ) )
						{
							...
							{
								mysql_commit();
								//echo"WENT INTO IF Stopped executing script, mysql was commited.";
								header("Location: reseller-home.php");
								die;
							}
							else
	   ...
	   if( $G_user_type == "ROOT" ) {
			...
			isset($_REQUEST["add_months"]) )
		{
			...
			$coverage_start = mktime(0, 0, 0,
						...
						substr($current_date, 6, 2),
						substr($current_date, 0, 4));
						//echo"HITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHIT";

			if($_REQUEST["add_months"] != 240) // Si no es demo
			{
				$coverage_end = $coverage_start + ((3600 * 24 * $G_days_per_month) * (int)$_REQUEST["add_months"]); //Le suma la cantidad de segundos en los meses seleccionados al Unix time de hoy
			...
			isset($_REQUEST["add_months"]) )
		{
			...
			$coverage_start = mktime(0, 0, 0,
						...
						substr($current_date, 6, 2),
						substr($current_date, 0, 4));
						//echo"HIT3HITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHIT";
			//echo"HITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHITHIT";
			if($_REQUEST["add_months"] != 240) // Si no es demo
			{
				$coverage_end = $coverage_start + ((3600 * 24 * $G_days_per_month) * (int)$_REQUEST["add_months"]); //Le suma la cantidad de segundos en los meses seleccionados al Unix time de hoy
				$mesesdbit = $_REQUEST["add_months"];
				 ...
				 $coverage_end = mktime(0, 0, 0,substr($current_date, 4, 2),substr($current_date, 6, 2)+1,substr($current_date, 0, 4));
				 $mesesdbit = 0; // No se va a agregar ningun saldo de debito porque la cuenta es DEMO
				 }
...
function add_demo_account()
{
	...
	if( root_form.add_demo_username.value != '' && root_form.add_demo_password.value != '')
	{
		if( !confirm('<?php grse("HOME_ADD_DEMO_CONFIRM"); ?>.') ) return;

		add_account_url = 'reseller-home.php?scrollLeft=' + escape(root_form.scrollLeft.value) + '&scrollTop=' +
			escape(root_form.scrollTop.value) +	'&__postback=1&add=1&add_username=' +
			escape(root_form.add_demo_username.value) + '&add_password=' + escape(root_form.add_demo_password.value) +
			'&add_comments=' + escape(root_form.add_demo_comments.value) +
			'&add_months=240' + '&show=' + root_form.show.value +
			'&order_by=' + root_form.order_by.value + '&page_index=' + root_form.page_index.value;
	...
	else alert('<?php echo"El usuario y contraseña no pueden estar en blanco." ?>.');
}
	...
	<tr>
		<td>
			...
			<table border="0" width="100%" id="tblSection_2">
				...
				<tr>
					<td width="25"></td>
				  <td colspan="2" class="listBackground"><table border="0" width="100%" box-shadow: 40px 40px 40px #3b3d3b>
                    <tr class="listHeader">
                      <td width="17%"><?php grse("HOME_LIST_SUBACCOUNT"); ?></td>
                      <td width="13%"><?php grse("HOME_LIST_PASSWORD"); ?></td>
                      <td width="13%"><?php grse("HOME_LIST_COMMENTS"); ?></td>
  ...
  if( srv_connchk() == "C" ) {
	...
	if ($showuseronline == "1" and isset($userinfo[$rw_account["account"]."-host"])) {
		...
		if ($activeusersstate[$printuser] == "1") {
			...
			<td width="9%"><font color="#006600"><b>
			
			<?php
			//$ip_addr = $userinfo[$rw_account["account"]."-ipaddress"];
			//$ip_addr = '187.190.161.120';
			...
			<?php
if(empty($userinfo[$rw_account["account"]."-servname"])) {
	$img = "images/.png";
} else {
	$img = "images/".$userinfo[$rw_account["account"]."-servname"].".png";
}

$canal = "<img src=\"{$img}\" height=\"30\" width=\"40\" title=\"".$userinfo[$rw_account["account"]."-servname"]."\">&nbsp;";
			
	
		echo "<td><font color=\"#006600\"><b>".$userinfo[$rw_account["account"]."-host"]."</b></font></td>";
		//echo "<td><font color=\"#006600\"><b>"."Joooooo"."</b></font></td>";
		echo "<td>".$canal."</td>";
		...
		echo "<td colspan=\"3\" align=\"center\"><font color=\"#0099FF\"><b>User STANBY></font></td>";
        } 
	  } else {
	    echo "<td colspan=\"3\" align=\"center\"><font color=\"#FF0000\"><b></b></font></td>";
	  } 
	} else {
